...
import { ProfileStudent, StudentArray, VehicleDTO } from "../../types/types";
...
export const StudentById: React.FC = () => {
  ...
  const [formData, setFormData] = useState<Partial<StudentArray>>({});
  ...
  const handleSave = async () => {
    ...
    try {
      if (formData.id) {
        await updateStudentById(uuid, formData as StudentArray);
        const updatedProfile = await getStudentById(uuid);
  ...
  const handleAddCar = async () => {
    if (!uuid) return;
    try {
      await addVehicleToStudent(uuid, carData as VehicleDTO);
      const updatedProfile = await getStudentById(uuid);
      setProfile(updatedProfile);
  ...
  const handleDeleteCar = async (vehicleId: string) => {
    try {
      ...
      if (uuid) {
        const updated = await getStudentById(uuid);
        setProfile(updated);
      }
    } catch {
  ...
  return (
    <div>
      ...
      <div className="profile-container">
        ...
        <p>
          <strong>ФИО:</strong> {profile.lastName} {profile.firstName}{" "}
          {profile.middleName}
        </p>
      ...
      {isModalOpen && (
        <div className="modal-overlay">
          <div className="modal">
            ...
            <input name="student_card" value={formData.student_card || ""} onChange={handleChange} placeholder="Студенческий билет" />
            <input name="faculty" value={formData.faculty || ""} onChange={handleChange} placeholder="Факультет" />
            <input name="course" value={formData.course || ""} onChange={handleChange} placeholder="Курс" />
            <input name="groups" value={formData.groups || ""} onChange={handleChange} placeholder="Группа" />
